...
#############################################################################
##
#F  GroupOfCodeRec( grprec ) . . . . . . . . . . . . . . . . . . . . .  local
##
##  The group of a code record, cached on the record.  The fingerprint step
##  and the random isomorphism test both need it, and decoding a code costs
##  about as much as one of the orbit computations below.
##  ReducedByIsomorphismsFEM drops the cache again.
##
BindGlobal( "GroupOfCodeRec", function( grprec )
    if not IsBound( grprec.grp ) then
        grprec.grp := PcGroupCodeRec( grprec );
    fi;
    return grprec.grp;
end );
...
#############################################################################
##
#F  FingerprintFEM( G ) . . . . . . . . . . . . . . . . . . . . . . . . local
##
##  As the library function FingerprintFF, but the element list is not stored
##  on G: a layer holds tens of thousands of cached groups, and one list of
##  |G| elements each outweighs everything else in the reduction.
##
BindGlobal( "FingerprintFEM", function( G )
    local orb, ord, res, po, i, typ;

    res := [ ];
    for orb in OrbitsDomain( G, AttributeValueNotSet( AsList, G ) ) do
        ord := Order( orb[ 1 ] );
        typ := [ ord, Length( orb ) ];
...
#############################################################################
##
#F  SplitUpByFingerprintFEM( list ) . . . . . . . . . . . . . . . . . . local
##
##  As the library function SplitUpSublistsByFpFunc, but the fingerprints are
##  kept sorted and located by binary search.  The linear scan of the library
##  version costs O( #groups * #fingerprints ), which becomes the dominant
##  term once a layer holds tens of thousands of groups.
##
BindGlobal( "SplitUpByFingerprintFEM", function( list )
   local result, sorted, idx, finp, i, j;
   ...
   for i in [ 1 .. Length( list ) ] do
      if list[ i ].isUnique then
         ...
         finp := FingerprintFEM( GroupOfCodeRec( list[ i ] ) );
         j := PositionSorted( sorted, finp );
         if j <= Length( sorted ) and sorted[ j ] = finp then
            Add( result[ idx[ j ] ], list[ i ] );
         else
            Add( result, [ list[ i ] ] );
            Add( sorted, finp, j );
            Add( idx, Length( result ), j );
            Info( InfoRandIso, 3, "split into ", Length( sorted ),
...
InstallGlobalFunction( RandomIsomorphismTestFEM, function( list, Finfo )
    local i, j, k, l, gi, ek, n, p,
          ...
          elms, eperm, t, ord, orb, orbs, orbss, tnorbss, norbss, fsize, exts,
    ...
    for gi in [ 1 .. n ]  do
        ...
        for i in [ 1 .. Length( Finfo.geninds ) ] do
            ...
            orbs := OrbitsDomain( g, Concatenation( t ) );
            typs := [];
            orbss := [];
            for orb in orbs do
                ord := Order( orb[ 1 ] );
            ...
            # a linear Position here would make the loop quadratic in the
            # number of elements examined; sort once and look up instead
            eperm := [ 1 .. Length( elms ) ];
            SortParallel( elms, eperm );
            ttyps := List( t, x -> Collected( List( x,
                     y -> inds[ eperm[ PositionSorted( elms, y ) ] ] ) ) );
            ...
            for orbs in orbss do
                ...
                for orb in orbs do
                    ...
                    typ := ttyps[ 1 + Int( ( eperm[ PositionSorted( elms,
                                             orb[ 1 ] ) ] - 1 ) / fsize ) ];
                    p := Position( typs, typ );
                    if p = fail then
                        Add( typs, typ );
...
InstallGlobalFunction( ReducedByIsomorphismsFEM, function( list, Finfo )
   ...
   # split up in sublist
   all  := list;
   list := SplitUpByFingerprintFEM( list );
   if ForAll( list, IsRecord ) then
      for i in all do Unbind( i.grp ); od;
      return list;
   fi;
